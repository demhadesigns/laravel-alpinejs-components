...
<div x-data="multiselect({
    ...
    <div class="flex items-center justify-between px-1 border border-2 rounded-md relative pr-8 bg-white">
        <ul class="flex flex-wrap items-center w-full">
            ...
            <!-- Search Input -->
            {{-- INFO: Added @keyup.prevent.stop to prevent form posting on hitting enter! --}}
            <input x-ref="searchInput" x-model="search" @click="expanded = true" @focusin="expanded = true"
                ...
                @keyup.arrow-up="expanded = true; selectPrevItem()" @keyup.escape="reset" @keydown.prevent.stop @keyup.enter="addActiveItem"
            ...
            <template x-if="filteredItems.length">
                <template x-for="(filteredItem, idx) in filteredItems">
                    <li x-text="shortenedLabel(filteredItem.label, maxItemChars)" @click="handleItemClick(filteredItem)"
                        :class="idx === activeIndex && 'bg-amber-200'" :title="filteredItem.label"
                        class="hover:bg-amber-200 cursor-pointer px-2 py-2"></li>
                </template>
            </template>
    ...
    {{-- If you need to make standard form posting you need this --}}
    <select class="hidden" multiple name={{ $name }}[]>
        <template x-for="item in selectedItems" :key="item.value">
            <option :value="item.label" selected></option>
        </template>
    </select>
</div>
    ...
    function multiselect(config) {
        return {
            ...
            shortenedLabel(label, maxChars) {
                return !maxChars || label.length <= maxChars ? label : `${label.substr(0, maxChars)}...`;
            },
